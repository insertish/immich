...
import { MaintenanceModeOperation } from 'src/types';
...
export interface ClientEventMap {
  ...
  MaintenanceOperationProgressV1: [
    {
      operation: MaintenanceModeOperation['operation'];
      progress: number;
    },
  ];
}
...
export class MaintenanceWorkerRepository implements OnGatewayConnection, OnGatewayDisconnect, OnGatewayInit {
  ...
  emitProgress(operation: MaintenanceModeOperation['operation'], progress: number) {
    this.websocketServer?.emit('MaintenanceOperationProgressV1', {
      operation,
      progress,
    });
  }
